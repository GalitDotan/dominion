...
class Pile(CardStructure):
    ...
    def __init__(self, cards: list[CardObject] = (), name: Optional[str] = None, is_visible: bool = True):
        super().__init__(cards, name, is_visible)

    def draw(self) -> CardObject | None:
        """
    ...
    def draw_all(self) -> list[CardObject]:
        """
        Draw all the cards from the pile.
        """
        return self.remove_all()

    def put(self, card: CardObject):
        """
        Put a card on top of the pile.
        """
        self.append(card)

    def put_all(self, cards: list[CardObject]):
        """
